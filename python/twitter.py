...
def get_tweets( latlong=None ):
	...
	results = twitter.search( geocode=','.join([ str(x) for x in latlong ]) + ',10km', result_type='recent', count=25 )
...
def get_lots_of_tweets( latlong ):
	...
	while remaining_seconds > 0:
		...
		for tweet in new_tweets:
			tid = tweet['id']
			geo = tweet['geo']
			if geo is not None and tid not in all_tweets:
...
if __name__=='__main__':
	...
	with open( './data/tweets_6.json', 'w' ) as f:
